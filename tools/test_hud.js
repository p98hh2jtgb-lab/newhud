...
check('FX dramatike default ON', iso.cfg.survivalHeartbeat && iso.cfg.survivalVignette && iso.cfg.survivalRedFlash &&
  iso.cfg.survivalImpactPopup && iso.cfg.survivalGameOver && iso.cfg.survivalCriticalGlitch);
check('zëri default = both', iso.cfg.survivalSoundMode === 'both');
check('overlay-at ekzistojnë', !!q('.pf-vignette') && !!q('.pf-flash') && !!q('.pf-impact'));
...
const cases = [[100, 'STEADY', 'safe', '#42ff68'], [70, 'TENSE', 'caution', '#ffd23f'],
               [40, 'IN PERIL', 'danger', '#ff7a2f'], [12, 'LAST BREATH', 'critical', '#ff334d'],
               [0, 'WRECKED', 'dead', '#b3122b']];
...
check('gameOverShow u aktivizua në 0%', iso.survival.gameOverShow === true);
check('overlay GAME OVER në DOM', txt('.pf-gameover') === 'GAME OVER', txt('.pf-gameover'));
$timeout.flush(5000);
check('GAME OVER fshihet vetë', iso.survival.gameOverShow === false);

console.log('\n— paketat e statusit —');
iso.setSurvivalPack('streamer'); $rootScope.$digest();
check('streamer 0% → SKILL ISSUE', iso.survivalStatus() === 'SKILL ISSUE', iso.survivalStatus());
...
check('dramatic 70% → TENSE', iso.survivalStatus() === 'TENSE', iso.survivalStatus());
...
check('vignette u ndez në 10%', iso.survivalVignetteStyle().opacity > 0.3, iso.survivalVignetteStyle().opacity);
setChance(100); iso.refreshSurvivalDrama(); $rootScope.$digest();
check('vignette fiket në 100%', iso.survivalVignetteStyle().opacity === 0);
...
check('FX-ja dramatike u ndez automatikisht', scope2.cfg.survivalHeartbeat && scope2.cfg.survivalRedFlash && scope2.cfg.survivalGameOver);
check('zëri u ndez automatikisht', scope2.cfg.survivalSoundMode === 'both');
check('fonti mbeti display (Bangers)', scope2.cfg.survivalFontFamily === 'futura');
